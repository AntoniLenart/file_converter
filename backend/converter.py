        ...
        while True:
            ...
            if export_filename:  # Jeśli plik został wybrany
                ...
                if messagebox.askyesno(
                    "Potwierdzenie", "Czy chcesz anulować zapisywanie pliku?"
                ):
...
def convert_video(input_file, input_format, output_format, infoLbl):
    ...
    try:
        ...
        if output_format == "mp4":
            video.write_videofile(export_filename, codec="libx264", audio_codec="aac")
        elif output_format == "mov":
            video.write_videofile(export_filename, codec="mpeg4")
        else:
            messagebox.showerror("Unsupported output format")
